...
This fork exposes a deliberately small **15-tool** catalog by default so an LLM
...
full **47-tool** Blockout catalog for advanced/manual operations.
...
Optional: `import_motion_previs_camera` turns a Motion Previs Studio `camera_motion.json` into editable Blockout camera marks before review/approval.
...
47 tools in full mode; director mode exposes 15. Coordinates are in **meters**: `+X` right, `−Z` forward/away from the default camera; **heading 0 faces −Z**; `rotationDeg` / `panDeg` are clockwise seen from above; `tiltDeg` is positive up. Focal lengths are mm on Super 35 (24 wide, 35 normal, 50–85 tight).
