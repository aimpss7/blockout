...
This fork exposes a deliberately small **16-tool** catalog by default so an LLM
...
full **48-tool** Blockout catalog for advanced/manual operations.
...
48 tools in full mode; director mode exposes 16. Coordinates are in **meters**: `+X` right, `−Z` forward/away from the default camera; **heading 0 faces −Z**; `rotationDeg` / `panDeg` are clockwise seen from above; `tiltDeg` is positive up. Focal lengths are mm on Super 35 (24 wide, 35 normal, 50–85 tight).
